...
/* =========================================================
   DOCTORS — filtered to the pin codes the executive (or the
   team, for manager/regional views) is assigned via
   pincode_coverage. A doctor only shows up here once an admin
   gives them a pincode that matches someone's coverage.
========================================================= */

function DoctorsSection({ data, role, execId, region }) {
  const { executives, coverage, doctors } = data;
  const [search, setSearch] = useState("");

  const execsInScope =
    role === ROLES.EXECUTIVE
      ? executives.filter((e) => e.id === execId)
      : region
      ? executives.filter((e) => e.region === region)
      : executives;
  ...
  const scopedDoctors = doctors.filter((d) => scopePincodes.includes(d.pincode));
  const q = search.trim().toLowerCase();
  const visibleDoctors = scopedDoctors.filter(
    (d) => !q || (d.name || "").toLowerCase().includes(q) || String(d.pincode || "").includes(q)
  );

  const isActive = (d) => d.is_active === "Yes" || d.is_active === true;
  const activeCount = scopedDoctors.filter(isActive).length;
  const rated = scopedDoctors.filter((d) => d.rating);
  const avgRating = rated.length ? (rated.reduce((s, d) => s + Number(d.rating || 0), 0) / rated.length).toFixed(1) : "—";

  return (
    <>
      <div className="stats">
        <Stat icon="▣" title="Pin Codes Assigned" value={scopePincodes.length} text={role === ROLES.EXECUTIVE ? "Your coverage" : "Team coverage"} type="blue" />
        <Stat icon="◎" title="Doctors In Area" value={scopedDoctors.length} text="Matching assigned pin codes" type="green" />
        <Stat icon="✓" title="Active Doctors" value={activeCount} text={`${scopedDoctors.length - activeCount} inactive`} type="orange" />
        <Stat icon="★" title="Avg. Rating" value={avgRating} text="Across doctors in area" type="red" />
      </div>

      <div className="panel">
        <div className="panel-title">
          <h2>Doctors in {role === ROLES.EXECUTIVE ? "Your" : "Team"} Pin Codes</h2>
          <input
        ...
        {scopePincodes.length === 0 ? (
          <p className="report-empty">No pin codes assigned yet — ask your manager to assign coverage.</p>
        ) : visibleDoctors.length === 0 ? (
          <p className="report-empty">No doctors found in the assigned pin codes yet.</p>
        ) : (
          <div className="doctor-grid">
            {visibleDoctors.map((doc) => (
              <div className="doctor-card" key={doc.id}>
                <div className="doctor-card-head">
                  <div className="person-avatar">{(doc.name || "?").charAt(0)}</div>
                  <div className="doctor-card-title">
                    <strong>{doc.name}</strong>
                    <small>{doc.qualification || "—"}</small>
                  </div>
                  <span className="doctor-pin-pill">PIN {doc.pincode || "—"}</span>
                </div>
                <div className="doctor-card-body">
                  <span>{doc.specializations || "General"}</span>
                  <span>{doc.experience_years ? `${doc.experience_years} yrs exp` : "— yrs exp"}</span>
                  <span>{doc.rating ? `★ ${doc.rating}` : "Unrated"}</span>
                </div>
                <div className="doctor-card-foot">
                  <span className={"doctor-status-pill " + (isActive(doc) ? "active" : "inactive")}>
                    {isActive(doc) ? "Active" : "Inactive"}
                  </span>
                  <span className="doctor-verification">{doc.verification_status || "pending"}</span>
                </div>
              </div>
            ))}
          </div>
...
/* =========================================================
   REPORTS — team call-coverage roll-up. Same shape as the
   teammate's Team Reports view (per-person expandable rows,
   coverage %, doctor list on expand) but rebuilt against our
   own schema and styled with our own panel/stat-card theme
   instead of Tailwind.
========================================================= */

function ReportsSection({ data, role, execId, region }) {
  ...
  const rows = execsInScope.map((exec) => {
    const pincodes = [...new Set(coverage.filter((c) => c.executive_id === exec.id).map((c) => c.pincode))];
    const myDoctors = doctors.filter((d) => pincodes.includes(d.pincode));
    const myTasks = tasks.filter((t) => t.pincode && pincodes.includes(t.pincode));
    const done = myTasks.filter((t) => t.status === "done").length;
    const pct = myTasks.length ? Math.round((done / myTasks.length) * 100) : 0;
    return { exec, pincodes, myDoctors, myTasks, done, pct };
  });

  const totalDoctors = new Set(rows.flatMap((r) => r.myDoctors.map((d) => d.id))).size;
  const totalTasks = rows.reduce((s, r) => s + r.myTasks.length, 0);
  ...
  return (
    <>
      <div className="stats">
        <Stat icon="♙" title={role === ROLES.EXECUTIVE ? "My Coverage" : "Team Size"} value={execsInScope.length} text={role === ROLES.EXECUTIVE ? "Executive" : "Executives in scope"} type="blue" />
        <Stat icon="◎" title="Doctors Covered" value={totalDoctors} text="Across assigned pin codes" type="green" />
        <Stat icon="▣" title="Tasks Completed" value={totalDone} text={`of ${totalTasks} total`} type="orange" />
        <Stat icon="₹" title="Completion Rate" value={`${overallPct}%`} text="Task completion" type="red" />
      </div>

      <div className="panel">
        <div className="panel-title">
          <h2>{role === ROLES.EXECUTIVE ? "My Report" : "Reports by Executive"}</h2>
        </div>

        <div className="report-list">
          {rows.length === 0 ? (
            <p className="report-empty">No executives in scope yet.</p>
          ) : (
            rows.map(({ exec, pincodes, myDoctors, myTasks, done, pct }) => {
              const isOpen = role === ROLES.EXECUTIVE || Boolean(expanded[exec.id]);
              return (
                <div className="report-row" key={exec.id}>
                  <button
                    type="button"
                    className="report-row-head"
                    onClick={() => role !== ROLES.EXECUTIVE && toggle(exec.id)}
                  >
                    {role !== ROLES.EXECUTIVE && <span className="report-caret">{isOpen ? "▾" : "▸"}</span>}
                    <div className="person-avatar">{(exec.name || "?").charAt(0)}</div>
                    <div className="report-row-info">
                      <strong>{exec.name}</strong>
                      <small>{exec.region || exec.city || "—"} · {pincodes.length} pin code{pincodes.length === 1 ? "" : "s"}</small>
                    </div>
                    <div className="report-row-meta">
                      <span>Coverage: <b>{done}</b>/<b>{myTasks.length}</b></span>
                      <span className={"report-pct-pill " + (pct >= 75 ? "good" : pct >= 40 ? "mid" : "low")}>{pct}%</span>
                    </div>
                  </button>
                  ...
                  {isOpen && (
                    <div className="report-row-body">
                      {pincodes.length > 0 && (
                        <div className="report-pincodes">
                          {pincodes.map((pc) => (
                            <span className="report-pincode-chip" key={pc}>PIN {pc}</span>
                          ))}
                        </div>
                      )}
                      {myDoctors.length === 0 ? (
                        <p className="report-empty">No doctors in this coverage area yet.</p>
                      ) : (
                        myDoctors.map((doc) => (
                          <div className="report-doctor-row" key={doc.id}>
                            <span className="report-doctor-name">{doc.name}</span>
                            <span className="report-doctor-meta">{doc.specializations || "General"} · PIN {doc.pincode}</span>
                            <span className={"doctor-status-pill " + (isActive(doc) ? "active" : "inactive")}>
                              {isActive(doc) ? "Active" : "Inactive"}
                            </span>
                          </div>
...
const VIEW_TITLES = {
  dashboard: null, // falls back to ROLE_TITLES
  doctors: "Doctors",
  reports: "Reports",
};
...
export default function SalesCrm({ role, onSwitchRole, onExit }) {
  ...
  const [view, setView] = useState("dashboard"); // "dashboard" | "doctors" | "reports"
    ...
    <div className="app">
      <aside className="sidebar">
        ...
        <nav>
          ...
          <button className={"nav-item" + (view === "doctors" ? " active" : "")} onClick={() => setView("doctors")}>Doctors</button>
      ...
      <main className="main">
        <header className="header">
          <div className="title-section">
            <div>
              <h1>{VIEW_TITLES[view] || ROLE_TITLES[role]}</h1>
        ...
        <section className="content">
          ...
          {!data.loading && !data.error && view === "dashboard" && role === ROLES.EXECUTIVE && (
            <ExecutiveDashboard data={data} execId={execId} />
          )}
          {!data.loading && !data.error && view === "dashboard" && role === ROLES.MANAGER && (
            <TeamDashboard data={data} region={region || null} scopeLabel="Active team members" />
          )}
          {!data.loading && !data.error && view === "dashboard" && role === ROLES.REGIONAL && (
            <TeamDashboard data={data} region={region || null} scopeLabel="Across all regions" />
          )}

          {!data.loading && !data.error && view === "doctors" && (
            <DoctorsSection data={data} role={role} execId={execId} region={region || null} />
          )}

          {!data.loading && !data.error && view === "reports" && (
            <ReportsSection data={data} role={role} execId={execId} region={region || null} />
          )}
        </section>
      </main>
